...
export async function createNotification(input: CreateNotificationInput) {
    ...
    return Notification.create({
        ...
        queueId:
        queueId && Types.ObjectId.isValid(queueId) ? new Types.ObjectId(queueId) : null,
...
export async function getUserNotificationsForUser(userId: string, unreadOnly: boolean = false) {
    if(!Types.ObjectId.isValid(userId)){
    ...
    return notifications.map((notification)=>({
        ...
        queueId: notification.queueId ? notification.queueId.toString() : null,
